...
def test_drill_function_file_exists():
    """drill_function.py must exist at the repo root."""
    assert Path("drill_function.py").exists(), "drill_function.py not found"


def test_clean_column_exists_and_works():
    """clean_column fills NaN values with the series median."""
    from drill_function import clean_column
...
def test_compute_revenue_exists_and_works():
    ...
    from drill_function import compute_revenue
    q = pd.Series([10, 20, 5])
